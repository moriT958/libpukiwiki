	...
	for {
		...
		if resp.StatusCode != http.StatusOK {
			if closeErr := resp.Body.Close(); closeErr != nil {
				return nil, fmt.Errorf("unexpected status code: %d (failed to close response body: %w)", resp.StatusCode, closeErr)
			}
		...
		if decodeErr != nil {
			if closeErr := resp.Body.Close(); closeErr != nil {
				return nil, fmt.Errorf("failed to parse response: %w (failed to close response body: %v)", decodeErr, closeErr)
			}
